...
import com.huishan.enjoytravel.databinding.FragmentMoveCarStatisticsBinding
...
/**挪车统计*/
@AndroidEntryPoint
class MovingStatisticsFragment: BaseFragment() {

    private var viewDataBinding: FragmentMoveCarStatisticsBinding? = null
    private val viewModel by viewModels<MovingStatisticsViewModel>()
    var adapter: MyBindingAdapter<MovingStatisticsEntity>? = null
    ...
    override fun onCreateView(
        ...
        viewDataBinding = FragmentMoveCarStatisticsBinding.bind(view).apply {
            viewmodel = viewModel
    ...
    override fun getLayoutResId(): Int {
        return R.layout.fragment_move_car_statistics
    }
    ...
    fun initList() {
        if (adapter == null) {
            adapter = MyBindingAdapter(requireContext(), R.layout.item_move_car_statistics, viewModel, viewModel.entityList)
    ...
    private fun observeLiveData() {
        viewModel.showDrop.observe(this) { type ->
            when (type) {
                ...
                2 -> {
                    initDropList(viewModel.allMoveCarList!!)
                }
            }
        }

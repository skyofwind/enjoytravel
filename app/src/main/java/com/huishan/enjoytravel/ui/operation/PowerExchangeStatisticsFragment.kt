...
import com.huishan.enjoytravel.databinding.FragmentChangeBatteryStatisticsBinding
...
/**换电统计*/
@AndroidEntryPoint
class PowerExchangeStatisticsFragment: BaseFragment() {

    private var viewDataBinding: FragmentChangeBatteryStatisticsBinding? = null
    private val viewModel by viewModels<PowerExchangeStatisticsViewModel>()
    var adapter: MyBindingAdapter<ItemPowerExchangeStatisticsEntity>? = null
    ...
    override fun onCreateView(
        ...
        viewDataBinding = FragmentChangeBatteryStatisticsBinding.bind(view).apply {
            viewmodel = viewModel
    ...
    override fun getLayoutResId(): Int {
        return R.layout.fragment_change_battery_statistics
    }
    ...
    fun initList() {
        if (adapter == null) {
            adapter = MyBindingAdapter(requireContext(), R.layout.item_change_battery_statistics, viewModel, viewModel.entityList)
    ...
    private fun observeLiveData() {
        viewModel.showDrop.observe(this) { type ->
            when (type) {
                ...
                2 -> {
                    initDropList(viewModel.allChangeList!!)
                }
            }
        }

...
import com.huishan.enjoytravel.data.remote.entity.ItemInspectionStatisticsEntity
import com.huishan.enjoytravel.databinding.FragmentInspectionStatisticsBinding
...
/**
 * 巡检统计
 * 筛选条件少写了一个，operation包的其他统计fragment可能也有这个情况，需要对照UI和原型补上
 */
@AndroidEntryPoint
class InspectionStatisticsFragment : BaseFragment() {

    private var viewDataBinding: FragmentInspectionStatisticsBinding? = null
    private val viewModel by viewModels<InspectionStatisticsViewModel>()
    var adapter: MyBindingAdapter<ItemInspectionStatisticsEntity>? = null
    ...
    override fun onCreateView(
        ...
        viewDataBinding = FragmentInspectionStatisticsBinding.bind(view).apply {
            viewmodel = viewModel
    ...
    fun initList() {
        if (adapter == null) {
            adapter = MyBindingAdapter(requireContext(), R.layout.item_inspection_statistics, viewModel, viewModel.entityList)
